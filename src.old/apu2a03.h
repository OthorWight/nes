...
typedef struct {
    ...
    // Noise Channel (Drums / Sound Effects)
    bool noise_enabled;
    bool noise_halt;
    ...
    // Audio downsampling buffers
    double audio_accumulator;
    float audio_buffer[4096];
    uint32_t audio_buffer_idx;

    // APU clock cycle toggle (for clocking at CPU_CLOCK / 2)
    bool clock_toggle;

    // Cycle counter for the 240Hz frame sequencer
