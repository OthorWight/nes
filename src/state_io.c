...
// Reflected IEEE CRC-32 (0xEDB88320), one byte per lookup. Keep this identical
// to the state-file checksum: diagnostics also use it on 240 KiB every frame.
...
uint32_t state_crc32(const void *data, size_t size) {
    ...
    for (size_t i = 0; i < size; ++i) {
        crc = (crc >> 8) ^ crc32_table[(crc ^ bytes[i]) & 0xFFu];
    }
    return ~crc;
}
