...
static void mmc4_write_prg(void *cart, uint16_t address, uint8_t data) {
    Cartridge *c = (Cartridge*)cart;
    if (address >= 0x6000 && address <= 0x7FFF) {
...
static uint8_t mmc4_read_chr(void *cart, uint16_t address) {
    Cartridge *c = (Cartridge*)cart;
...
static void mmc4_write_chr(void *cart, uint16_t address, uint8_t data) {
    Cartridge *c = (Cartridge*)cart;
...
void mapper_010_init(Cartridge *cart) {
    ...
    cart->write_prg = mmc4_write_prg;
    cart->read_chr = mmc4_read_chr;
    cart->write_chr = mmc4_write_chr;
